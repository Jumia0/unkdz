...
sys.path.append(os.path.join(os.path.dirname(__file__), "libexec"))
...
class UNDZUtils(object):
    ...
    def loadHeader(self, file):
        ...
        if dz_item == None:
            print("[!] Bad DZ {:s} header!".format(
                self._dz_area), file=sys.stderr)
            sys.exit(1)
        ...
        for key in self._dz_collapsibles:
            if type(dz_item[key]) is str or type(dz_item[key]) is bytes:
                ...
                if b'\x00' in dz_item[key]:
                    print("[!] Warning: extraneous data found IN " +
                          key, file=sys.stderr)
                    # sys.exit(1)
            elif type(dz_item[key]) is int:
                if dz_item[key] != 0:
                    print('[!] Error: Value supposed to be zero in field "'+key +
                          '" is non-zero ('+hex(dz_item[key])+')', file=sys.stderr)
                    sys.exit(1)
            else:
...
class UNDZChunk(dz.DZChunk, UNDZUtils):
    ...
    def display(self, sliceIdx, selfIdx):
        ...
        if cmd.batchMode:
            print("{:d}:{:s}:data".format(
                sliceIdx, self.sliceName.decode("utf8")))
        else:
            print("{:2d}/{:2d} : {:s} ({:d} bytes)".format(sliceIdx,
    ...
    #               sys.exit(1)
        ...
        if md5.digest() != self.md5:
            print("[!] Error: MD5 of data doesn't match header ({:32s} vs {:32s})".format(
                md5.hexdigest(), b2a_hex(self.md5)), file=sys.stderr)
            sys.exit(1)
    ...
    def extractChunk(self, file, name):
        ...
        if name:
            print(
                "[+] Extracting {:s} to {:s}".format(self.chunkName.decode("utf8"), name))
...
# experiment, results negative
        ...
        if dz_item['targetSize'] & 0x1FF != 0:
            self.messages.append(
                "[?] Warning: uncompressed size is {:d}, not a multiple of 512 (please report!)".format(dz_item['targetSize']))
...
class UNDZSlice(object):
    ...
    def addChunk(self, chunk):
        ...
        if offset < self.start:
            print("[!] Warning: Chunk is part of \"{:s}\", but starts in front of slice?!".format(
    ...
    def extractSlice(self, file, name):
        ...
        params.write(
            u"# this value may be crucial for success and dangerous to modify\n")

        if len(self.chunks) > 0:
            last = self.chunks[-1]
            params.write(u"lastWipe={:d}\n".format(
                (last.getTargetStart() >> self.dz.shiftLBA) + last.trimCount))
            params.write(
                u"# Indicates which flash device this should be written in\n")
...
class UNDZFile(dz.DZFile, UNDZUtils):
    ...
    def open(self, name):
        ...
        if dz_file['formatMajor'] > 2:
            print(
                "[!] Error: DZ format version too high! (please report)", file=sys.stderr)
            sys.exit(1)
        elif dz_file['formatMinor'] > 1:
            print(
                "[!] Warning: DZ format more recent than previous versions, output unreliable", file=sys.stderr)
    ...
    def loadChunks(self):
        ...
        if disorder > 0:
            print("[ ] Warning: Found {:d} out of order chunks (please report)".format(
                disorder), file=sys.stderr)

        # They're in the order to write, not block order though
        self.chunks.sort(key=lambda c: (
            c.getTargetStart() + (c.getDev() << 48)))

        try:
            emptycount = 0
            g = gpt.GPT(self.chunks[0].extract())
            ordered = range(len(g.slices)) if g.ordered else range(
                len(g.slices)).sort(key=lambda s: g.slices[s].startLBA)

            self.shiftLBA = g.shiftLBA

            next = g.dataStartLBA
            slice = UNDZSlice(
                self, 0, self.chunks[0].getSliceName(), 0, next << self.shiftLBA)
            ...
            for slice in g.slices:
                new = UNDZSlice(self, index, slice.name, slice.startLBA <<
                                self.shiftLBA, (slice.endLBA+1) << self.shiftLBA)
            ...
            for i in range(len(g.slices)):
                if next != g.slices[i].startLBA:
                    print("[!] Unallocated space found. Slice, Start, Size, End: " + str(next) + " " + str(
                        (g.slices[i].startLBA - next) << self.shiftLBA), next << self.shiftLBA, (g.slices[i].startLBA-1) << self.shiftLBA)
                    emptycount += 1
                next = g.slices[i].endLBA+1

            if next != g.dataEndLBA+1:
                new = UNDZSlice(self, None, "_unallocated_" + str(emptycount),
                                next << self.shiftLBA, g.dataEndLBA << self.shiftLBA)
                self.slices.append(new)
                emptycount += 1
                next = g.dataEndLBA+1

            slice = UNDZSlice(self, index, self.chunks[-1].getSliceName(
            ), g.dataEndLBA << self.shiftLBA, (g.altLBA+1) << self.shiftLBA)
            self.slices.append(slice)
    ...
    def checkValues(self):
        ...
        if len(self.chunks) != self.chunkCount:
            print(
                "[!] Error: chunks in header differs from chunks found (please report)", file=sys.stderr)
            sys.exit(-1)
        ...
        if md5Headers != self.md5:
            print("[!] Error: MD5 of chunk headers doesn't match header ({:32s} vs {:32s})".format(
                self.md5Headers.hexdigest(), b2a_hex(self.md5)), file=sys.stderr)
            sys.exit(-1)
        ...
        if md5HeaderNZ == self.unknown2:
            self.messages.add(
                "[ ] unknown2 is consistent with MD5 of non-zero header areas")
        else:
            self.messages.add(
                "[ ] MD5 of non-zero header areas not found ({:32s})".format(self.md5HeaderNZ.hexdigest()))

        self.crcHeaders = self.crcHeaders & 0xFFFFFFFF
        if self.crcHeaders == self.unknown0:
            self.messages.add(
                "[ ] unknown0 is consistent with CRC32 of headers")
        elif self.crcHeaders == self.unknown1:
            self.messages.add(
                "[ ] unknown1 is consistent with CRC32 of headers")
        elif self.crcHeaders == self.unknown4:
            self.messages.add(
                "[ ] unknown4 is consistent with CRC32 of headers")
        else:
            self.messages.add(
                "[ ] No CRC32 of headers found ({:08X})".format(self.crcHeaders))

        self.crcHeaderNZ = self.crcHeaderNZ & 0xFFFFFFFF
        if self.crcHeaderNZ == self.unknown0:
            self.messages.add(
                "[ ] unknown0 is consistent with CRC32 of non-zero header areas")
        elif self.crcHeaderNZ == self.unknown1:
            self.messages.add(
                "[ ] unknown1 is consistent with CRC32 of non-zero header areas")
        elif self.crcHeaderNZ == self.unknown4:
            self.messages.add(
                "[ ] unknown4 is consistent with CRC32 of non-zero header areas")
        else:
            self.messages.add(
                "[ ] No CRC32 of non-zero header areas found ({:08X})".format(self.crcHeaders))

        if self.unknown0 == 256:
            self.messages.add(
                "[ ] unknown 0 is 256 like always (half blocksize?)")
    ...
    def saveHeader(self, name):
        ...
        params.write("unknown3={:s}\n".format(
            b2a_hex(self.unknown3).decode("utf8")))
...
class DZFileTools:
    ...
    def parseArgs(self):
        ...
        parser = argparse.ArgumentParser(
            description='LG Compressed DZ File Extractor originally by IOMonster')
        parser.add_argument('-f', '--file', help='DZ File to read',
                            action='store', required=True, dest='dzfile')
        parser.add_argument('-b', '--batch', help='batch mode',
                            action='store_true', dest='batchMode')
        ...
        group.add_argument('-c', '--chunk', help='extract data chunk(s) (all by default)',
                           action='store_true', dest='extractChunk')
        group.add_argument('-s', '--single', help='extract diskslice(s) (partition(s)) (all by default)',
                           action='store_true', dest='extractSlice')
        group.add_argument('-i', '--image', help='extract all slices/partitions as a disk image',
                           action='store_true', dest='extractImage')
        parser.add_argument('-d', '--dir', '-o', '--out',
                            help='output location', action='store', dest='outdir')
        ...
        for idx in files:
            ...
            if idx < 0 or idx >= self.dz_file.getChunkCount():
                print("[!] Cannot extract out of range chunk {:d} (min=0 max={:d})".format(
                    idx, self.dz_file.getChunkCount()-1), file=sys.stderr)
                sys.exit(1)
    ...
    def cmdExtractChunkfile(self, files):
        ...
        for idx in files:
            try:
                ...
                sys.exit(1)
            if idx < 0 or idx >= self.dz_file.getChunkCount():
                print("[!] Cannot extract out of range chunkfile {:d} (min=0 max={:d})".format(
                    idx, self.dz_file.getChunkCount()-1), file=sys.stderr)
                sys.exit(1)
    ...
    def cmdExtractSlice(self, files):
        if len(files) == 0:
            ...
            print(
                "[+] Extracting {:d} slices^Wpartitions!\n".format(len(files)))
        ...
        for idx in files:
            ...
            if idx < 0 or idx > self.dz_file.getSlice(-1).getIndex():
                print("[!] Cannot extract out of range slice {:d} (min=0 max={:d})".format(
                    idx, self.dz_file.getSlice(-1).getIndex()), file=sys.stderr)
                sys.exit(1)
    ...
    def cmdExtractImage(self, files):
        if len(files) > 0:
            print(
                "[!] Cannot specify specific portions to extract when outputting image", file=sys.stderr)
            sys.exit(1)
        name = "image.img"
        try:
    ...
    def run(self, dzfile, outdir, listOnly=0, files=None, extractSlice=0, extractChunkfile=0, extractImage=0, extractChunk=0, rawprogram=0):
